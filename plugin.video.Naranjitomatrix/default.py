...
url_principal = 'https://raw.githubusercontent.com/brujotv/puertotv/master/inicio.xml'
...
# --- FUNCIÓN PARA LEER Y BUSCAR EN EL XML ---
def cargar_peliculas_xml(ruta_xml):
    peliculas = []
    ...
    for item in root.findall('item'):
        pelicula = {
            'title': item.findtext('title', '').replace('[COLOR lime]•[/COLOR] ', '').strip(),
            'link': item.findtext('link', '').strip(),
            'thumbnail': item.findtext('thumbnail', '').strip(),
            'fanart': item.findtext('fanart', '').strip(),
            'genre': item.findtext('genre', '').strip(),
            'year': item.findtext('year', '').strip(),
            'info': item.findtext('info', '').strip()
        }
...
def buscar_peliculas():
    ruta_xml = os.path.join(xbmcaddon.Addon().getAddonInfo('path'), '..', 'full.xml')
    ruta_xml = os.path.abspath(ruta_xml)
    peliculas = cargar_peliculas_xml(ruta_xml)
    if not peliculas:
        xbmcgui.Dialog().ok('Error', 'No se encontró el archivo full.xml o está vacío.')
        return

    opciones = ['Nombre', 'Género', 'Año']
    tipo = xbmcgui.Dialog().select('Buscar por...', opciones)
    if tipo == -1:
        return

    texto = xbmcgui.Dialog().input('Introduce el texto de búsqueda:')
    if not texto:
        return

    texto = texto.lower()
    if tipo == 0:  # Nombre
        resultados = [p for p in peliculas if texto in p['title'].lower()]
    elif tipo == 1:  # Género
        resultados = [p for p in peliculas if texto in p['genre'].lower()]
    elif tipo == 2:  # Año
        resultados = [p for p in peliculas if texto in p['year'].lower()]
    else:
        resultados = []

    if not resultados:
        xbmcgui.Dialog().ok('Sin resultados', 'No se encontraron películas.')
        return
    ...
    for peli in resultados:
        addDir(
            peli['title'],
            peli['link'],
            1,  # Modo de reproducción estándar
            peli['thumbnail'],
            peli['fanart'],
            peli['info'],
            peli['genre'],
...
# --- AGREGAR OPCIÓN AL MENÚ PRINCIPAL ---
# Busca el bloque donde se agregan las opciones principales con addDir y añade:
# addDir('[B][COLOR yellow]Buscar películas[/COLOR][/B]', 'buscar_peliculas', 1001, '', '', 'Buscar por nombre, género o año', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', True)
#
# Por ejemplo, si tienes algo como:
# addDir('Películas', 'url', 1, ...)
#
# Añade justo después:
try:
    if len(sys.argv) == 1 or (len(sys.argv) > 1 and sys.argv[2] == ''):
        addDir('[B][COLOR yellow]Buscar películas[/COLOR][/B]', 'buscar_peliculas', 1001, '', '', 'Buscar por nombre, género o año', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', True)
except:
    pass
# ... código existente ...
# --- MANEJO DEL MODO DE BÚSQUEDA ---
# En el bloque de control de modos, añade:
if 'mode' in locals() and mode == 1001:
    buscar_peliculas()
    exit()